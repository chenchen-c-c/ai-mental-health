...
export default function KnowledgeDetailClient({ id }) {
  ...
  useEffect(() => {
    ...
    const foundArticle = allArticles.find(a => a.id === parsedId);
    setArticle(foundArticle);

    if (foundArticle) {
      const otherArticles = allArticles.filter(a => a.id !== parsedId && a.status === 'published');
